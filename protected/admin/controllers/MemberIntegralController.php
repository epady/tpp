...
class MemberIntegralController extends Controller
{
	...
	{
		$form = new Form('backend.views.memberIntegral.formConfig');
	    $form->model = new MemberIntegral;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
			...
			*/
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', MemberIntegral::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
				...
				/**
		        	...
		        	*/		        	
		        	Yii::app()->user->setFlash('success',MemberIntegral::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
			/**
	...
	{
		$model=new MemberIntegral('search');
		$model->unsetAttributes();
		if(isset($_GET['MemberIntegral']))
			$model->attributes=$_GET['MemberIntegral'];
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = MemberIntegral::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=MemberIntegral::model()->findbyPk($_GET['id']);
			if($this->_model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='member-integral-form')
		{
