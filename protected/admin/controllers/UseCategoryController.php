...
class UseCategoryController extends Controller
{
	...
	{
		$form = new Form('backend.views.useCategory.formConfig');
	    $form->model = new UseCategory;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
			...
			*/
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', UseCategory::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
				...
				/**
		        	...
		        	*/		        	
		        	Yii::app()->user->setFlash('success',UseCategory::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
			/**
	...
	{
		$model=new UseCategory('search');
		$model->unsetAttributes();
		if(isset($_GET['UseCategory']))
			$model->attributes=$_GET['UseCategory'];
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = UseCategory::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=UseCategory::model()->findbyPk($_GET['id']);
			if($this->_model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='use-category-form')
		{
