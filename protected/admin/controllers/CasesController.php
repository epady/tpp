...
class CasesController extends Controller
{
	...
	{
		$form = new Form('backend.views.cases.formConfig');
	    $form->model = new Cases;
	    ...
	    if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	Yii::app()->user->setFlash('success', Cases::model()->modelName."“{$model}”已成功添加！");
		...
		if($form->submitted('btnsubmit'))
	    {
	    	...
	    	if($form->validate())
	    	{
		        ...
		        if($model->save(false))
		        {
		        	...
		        	Yii::app()->user->setFlash('success',Cases::model()->modelName."“{$model}”已成功修改！");
					$this->redirect(array('view','id'=>$model->id));
				}
	    	}
	    	else 
	...
	public function actionOperate()
	{
		if(Yii::app()->request->isAjaxRequest && !empty($_POST['id']))
		{
			...
			{
				$objs = Cases::model()->findAllByPk($_POST['id']);
				if($objs)
				{
					foreach($objs as $obj)
					{
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=Cases::model()->findbyPk($_GET['id']);
			if($this->_model===null)
